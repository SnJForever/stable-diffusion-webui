...
def download_url_check_to_file(url,path,hash_prefix,progress):
    if not os.path.exists(path):
        download_url_to_file(url, path, hash_prefix, progress)
    else:
        print(path,'已经存在')
...
download_url_check_to_file('https://civitai.com/api/download/models/36644', MODEL_PATH+'/3Guofeng3_v33.safetensors', hash_prefix=None, progress=True)

# https://civitai.com/models/26578/xsarchitectural-17sciencefictioncityonmars
download_url_check_to_file('https://civitai.com/api/download/models/31884', MODEL_PATH+'/xsarchitectural_v11.safetensors', hash_prefix=None, progress=True)
